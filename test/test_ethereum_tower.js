...
describe("EthereumTower contract", function () {
  ...
  it("should revert changeTower to unsupported tower", async function () {
    ...
    const ethereumTowers = await contractFactory.attach(nft)
  ...
  describe("Ethereum Tower 1", function () {
    ...
    before(async () => {
      ...
      ethereumTowers = await contractFactory.attach(nft);
    })
    ...
    it("should add WHITELISTED role to stage 0", async function () {
      let whitelistedRole = await ethereumTowers.WHITELISTED();
    ...
    it("should restrict calling addStageRole to admin role only", async function () {
      ...
      await expect(ethereumTowers.connect(testUser).addStageRole(0, whitelistedRole)).to.be.revertedWith('EthereumTowers: must have admin role');
    })
